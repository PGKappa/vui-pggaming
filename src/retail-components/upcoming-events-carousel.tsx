...
export function UpcomingEventsCarousel(props: {
  ...
  const disciplines = useMemo(() => {
    ...
    if (path.includes('dogs8')) {
      return [Discipline.DOGS8]
    } else if (path.includes('dogs-horses')) {
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col items-start pr-[1px]">
        <span className="relative top-[4px] font-semibold text-[15]">
          {event.discipline === 'SOCCER'
            ? event.name
            : `${event.name} ${t('racing')}`}
        </span>

        <span className="relative bottom-[0px] text-[14px] font-normal">
        ...
        <div className="flex flex-row gap-2">
          ...
          <span className="absolute bottom-[3px] py-0 pl-[45px] text-[14px] font-semibold text-[#99a6b1]">
            {timeToEventStart}
          </span>
        </div>
      </div>
