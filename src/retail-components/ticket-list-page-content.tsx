...
export default function TicketListPageContent({
  ...
  const [selectedTicketCandidates, setSelectedTicketCandidates] = useState<Array<string | number>>([])
  ...
  const buildTicketCandidates = (item: { ticket_id: number }) => {
    ...
    const candidates = [
      raw.ticket_code, raw.code, raw.ticketId,
      raw.ext_ticket_id, raw.ticket_ref, item.ticket_id,
    ]
  ...
  const handleDetailsClick = (item: { ticket_id: number }) => {
    ...
    setSelectedTicketId(
      typeof primary === 'number'
        ? primary
        : Number.isFinite(fallbackNumber) ? fallbackNumber : null,
    )
    setDialogOpen(true)
  }
  ...
  const calendarClassNames = {
    ...
    day_selected: 'bg-red-700 text-white hover:bg-red-800 focus:bg-red-700 focus:text-white rounded-md',
    day_today: 'bg-red-700 text-white rounded-md',
    day_outside: 'text-gray-300 opacity-50',
    day_disabled: 'text-gray-300 opacity-30',
    caption_label: 'text-sm font-semibold text-gray-800',
    nav_button: 'h-7 w-7 bg-transparent p-0 opacity-70 hover:opacity-100 border border-gray-300 rounded',
  }
        ...
        <div className="flex flex-col items-center gap-4 px-4 pb-8 pt-10">
          <div className="flex items-center gap-8">
              ...
              <Select value={terminal} onValueChange={setTerminal}>
                ...
                <SelectContent className="bg-white p-0">
                  ...
                  {availableTerminals.map((tid) => (
                    <SelectItem key={tid} value={tid}>{tid}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            </div>
            ...
            <div className="mr-20 flex items-center gap-4">
              ...
              <div className="flex flex-row items-center gap-2 bg-badge text-background">
                <span className="whitespace-nowrap pl-2 text-[12px] font-semibold">{t('payment')}</span>
            ...
            <div className="mr-20 flex flex-row items-center gap-2 bg-badge text-background">
              <span className="whitespace-nowrap pl-2 text-[12px] font-semibold">{t('page_size')}</span>
        ...
        <div className="flex shrink-0 justify-center bg-secondary pb-3 lg:pb-5">
          <div className="relative top-2 flex flex-wrap items-center space-x-5 lg:top-2.5 lg:space-x-10">
            ...
            <div className="flex flex-row items-center space-x-1 bg-accent text-background lg:space-x-2">
              <span className="whitespace-nowrap pl-1 text-[10px] font-semibold lg:pl-2 lg:text-[12px]">{t('terminal')}</span>
              <Select value={terminal} onValueChange={setTerminal}>
            ...
            <div className="flex flex-row items-center space-x-1 bg-accent text-background lg:space-x-2">
              <span className="whitespace-nowrap pl-1 text-[10px] font-semibold lg:pl-2 lg:text-[12px]">{t('status')}</span>
              <Select value={status} onValueChange={setStatus}>
                <SelectTrigger className="h-7 w-[70px] bg-background text-[10px] text-foreground lg:h-9 lg:w-[100px] lg:text-[12px]">
                  <SelectValue placeholder={t('status')} />
                </SelectTrigger>
            ...
            <div className="flex flex-row items-center space-x-1 bg-accent text-background lg:space-x-2">
              <span className="whitespace-nowrap pl-1 text-[10px] font-semibold lg:pl-2 lg:text-[12px]">{t('payment')}</span>
              <Select value={payment} onValueChange={setPayment}>
                <SelectTrigger className="h-7 w-[70px] bg-background text-[10px] text-foreground lg:h-9 lg:w-[100px] lg:text-[12px]">
                  <SelectValue placeholder={t('payment')} />
                </SelectTrigger>
...
<div className="flex flex-row items-center space-x-1 bg-accent text-background lg:space-x-2">
  ...
  {dateToButton('h-7 w-[80px] justify-center text-[10px] lg:h-9 lg:w-[100px] lg:text-[12px]')}
            ...
            <Button onClick={fetchTickets} className="text-bold h-7 w-[60px] bg-tertiary text-[10px] text-tertiary-foreground lg:h-9 lg:w-[80px] lg:text-[14px]">
              {t('reload')}
            </Button>
          </div>
        </div>
      ...
      <div className="flex-1 overflow-auto bg-white text-black">
        <table className={cn('w-full', isCalcio ? 'text-[12px]' : '')}>
          <thead className={cn('bg-secondary text-white', isCalcio ? '' : 'sticky top-0 z-10')}>
            <tr>
              <th className={cn('text-[16px]', isCalcio ? 'bg-badge p-2' : 'bg-accent p-1 text-[12px] lg:p-2 lg:text-[16px]')}>{t('ticket_id')}</th>
              <th className="w-[1px] bg-card-header-foreground p-0"></th>
              <th className={cn('text-[16px]', isCalcio ? 'bg-badge p-2' : 'bg-accent p-1 text-[12px] lg:p-2 lg:text-[16px]')}>{t('date_n_time')}</th>
              <th className="w-[1px] bg-card-header-foreground p-0"></th>
              <th className={cn('text-[16px]', isCalcio ? 'bg-badge p-2' : 'bg-accent p-1 text-[12px] lg:p-2 lg:text-[16px]')}>{t('terminal')}</th>
              <th className="w-[1px] bg-card-header-foreground p-0"></th>
              <th className={cn('text-[16px]', isCalcio ? 'bg-badge p-2' : 'bg-accent p-1 text-[12px] lg:p-2 lg:text-[16px]')}>{t('product')}</th>
              <th className="w-[1px] bg-card-header-foreground p-0"></th>
              <th className={cn('text-[16px]', isCalcio ? 'bg-badge p-2' : 'bg-accent p-1 text-[12px] lg:p-2 lg:text-[16px]')}>{t('staked_amount')}</th>
              <th className="w-[1px] bg-card-header-foreground p-0"></th>
              <th className={cn('text-[16px]', isCalcio ? 'bg-badge p-2' : 'bg-accent p-1 text-[12px] lg:p-2 lg:text-[16px]')}>{t('won')}</th>
              <th className="w-[1px] bg-card-header-foreground p-0"></th>
              <th className={cn('text-[16px]', isCalcio ? 'bg-badge p-2' : 'bg-accent p-1 text-[12px] lg:p-2 lg:text-[16px]')}>{t('ticket_status')}</th>
              <th className="w-[1px] bg-card-header-foreground p-0"></th>
              <th className={cn('text-[16px]', isCalcio ? 'bg-badge p-2' : 'bg-accent p-1 text-[12px] lg:p-2 lg:text-[16px]')}>{t('payment')}</th>
              <th className="w-[1px] bg-card-header-foreground p-0"></th>
              <th className={cn('text-[16px]', isCalcio ? 'bg-badge p-2' : 'bg-accent p-1 text-[12px] lg:p-2 lg:text-[16px]')}></th>
            </tr>
          </thead>
          <tbody>
            {loading ? (
              <tr><td colSpan={17} className="p-8 text-center text-gray-400">{t('loading')}...</td></tr>
            ) : items.length === 0 ? (
              <tr><td colSpan={17} className="p-8 text-center text-gray-400">{t('no_tickets_found')}</td></tr>
            ) : (
              items.map((item) => {
                const date = parseTicketTime(item.time)
                const statusInfo = getStatusDisplay(item.status)
                return (
                  <tr key={item.ticket_id} className={cn('border-b text-center', isCalcio ? 'text-[16px]' : 'text-[12px] lg:text-[16px]')}>
                    <td className={cn(isCalcio ? 'p-2' : 'p-1 lg:p-2')}>{item.ticket_id}</td>
                    <td className="w-[1px] bg-muted p-0"></td>
                    <td className={cn(isCalcio ? 'p-2' : 'p-1 lg:p-2')}>{format(date, 'dd/MM/yy')} - {format(date, 'HH:mm:ss')}</td>
                    <td className="w-[1px] bg-muted p-0"></td>
                    ...
                    <td className={cn(isCalcio ? 'p-2' : 'p-1 lg:p-2')}>
                      <div className={cn('flex items-center justify-center', isCalcio ? 'gap-2' : 'space-x-1 lg:space-x-2')}>
                        <div className={cn(isCalcio ? 'h-3 w-3 rounded-sm' : 'h-2 w-2 rounded-sm lg:h-3 lg:w-3', statusInfo.colorClass)} />
                        <span className={cn('font-medium', isCalcio ? 'text-[16px]' : '')}>{t(statusInfo.translationKey)}</span>
                      </div>
                    </td>
                    <td className="w-[1px] bg-muted p-0"></td>
                    <td className={cn(isCalcio ? 'p-2' : 'p-1 lg:p-2')}>
                      {parseFloat(item.amount_won) > 0 && item.status === 4 ? t('unpaid') : '-'}
                    </td>
                    <td className="w-[1px] bg-muted p-0"></td>
                    <td className={cn(isCalcio ? 'p-2' : 'p-1 lg:p-2')}>
                      <Button
                        onClick={() => handleDetailsClick(item)}
                        className={cn('bg-tertiary text-tertiary-foreground', isCalcio ? 'h-8 w-20 text-[16px]' : 'h-6 w-14 text-[10px] lg:h-8 lg:w-20 lg:text-[16px]')}
                      >
                        {t('details')}
                      </Button>
                    </td>
        ...
        <table className={cn(isCalcio ? 'col-span-7 border-collapse' : 'col-span-7 h-full border-collapse')}>
          <tbody>
            ...
            <tr className="bg-accent text-xs font-medium text-white">
              ...
              <td className={cn('border border-muted bg-accent text-center align-middle', isCalcio ? 'px-3 py-2' : 'px-1 py-1 text-[10px] lg:px-3 lg:py-2 lg:text-md')}>{formatCurrency(info?.grandtotal?.cancelled ?? '0.00', currencySymbol)}</td>
              <td className={cn('border border-muted bg-accent text-center align-middle', isCalcio ? 'px-3 py-2' : 'px-1 py-1 text-[10px] lg:px-3 lg:py-2 lg:text-md')}>{formatCurrency(info?.grandtotal?.out ?? '0.00', currencySymbol)}</td>
              <td className={cn('border border-muted bg-accent text-center align-middle', isCalcio ? 'px-3 py-2' : 'px-1 py-1 text-[10px] lg:px-3 lg:py-2 lg:text-md')}>{formatCurrency('0.00', currencySymbol)}</td>
              <td className={cn('border border-muted bg-accent text-center align-middle', isCalcio ? 'px-3 py-2' : 'px-1 py-1 text-[10px] lg:px-3 lg:py-2 lg:text-md')}>{info?.count_paid ?? 0} / {info?.count_won ?? 0}</td>
              <td className={cn('border border-muted bg-accent text-center align-middle', isCalcio ? 'px-3 py-2' : 'px-1 py-1 text-[10px] lg:px-3 lg:py-2 lg:text-md')}>{info?.count ?? 0}</td>
            </tr>
          </tbody>
        </table>
      </div>
